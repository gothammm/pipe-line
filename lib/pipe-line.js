...
Pipeline.prototype.log = function log() {
  ...
  if (_self.currentTask && _self.currentTask.options && _self.currentTask.options.logs === false) {
    return false;
  }
  let loggerMethod = ((_self.currentTask &&
    _self.currentTask.options &&
    _self.currentTask.options.loggerMethod)
    || _self.options.loggerMethod || 'info');

  let logger = (_self._log && _self._log[loggerMethod || 'info']) || console.log;
  return logger(args.join(' ') || '');
};
